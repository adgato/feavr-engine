...
namespace assets_system
{
    ...
    std::vector<std::string> ShaderAssetManager::GenerateAssets(const std::string& assetPath, std::vector<std::byte>&& contents)
    {
        ...
        for (const auto& [entry, profile] : detectedEntries)
        {
            ...
            metadata["stem"] = assetFilePath.stem().string();
