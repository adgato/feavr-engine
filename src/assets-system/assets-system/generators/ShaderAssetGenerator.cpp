...
std::vector<std::byte> compile_shader(const std::string& dxcCompiler, const std::string& entry, const std::string& profile, const std::string& hlslFile)
{
    ...
    if (exitCode != 0)
    {
        ...
        fmt::println(stderr, "DXC compilation failed for file: {}\n{}", hlslFile, errorText);
        return {};
    }
    ...
    {
        std::filesystem::remove(tempError);
        fmt::println(stderr, "SPIR-V output file not created: {}", tempSpv);
        return {};
    }
    ...
    if (!spvFile.is_open())
    {
        std::filesystem::remove(tempSpv);
        std::filesystem::remove(tempError);
        fmt::println(stderr, "Failed to open compiled SPIR-V file: {}", tempSpv);
        return {};
    }
