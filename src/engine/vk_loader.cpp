...
void loadPrimitiveVertices(const fastgltf::Asset& gltf, const fastgltf::Primitive& primitive, std::vector<Vertex>& vertices)
{
    ...
    for (size_t i = 0; i < posAccessor.count; ++i)
    {
        ...
        Vertex& vertex = vertices[vertexOffset + i];
...
void loadGltf(Core* core, std::string_view filePath)
{
    ...
    std::vector meshOffset = {static_cast<uint32_t>(passManager.meshes.size())};
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes)
    {
        ...
        auto fullmesh = std::make_shared<rendering::Mesh>(core->engine.UploadMesh(&core->swapchain, indices, vertices));

        uint32_t cumCount = 0;
        for (auto&& p : mesh.primitives)
        {
            rendering::SubMesh& newmesh = passManager.meshes.emplace_back();

            uint32_t count = static_cast<uint32_t>(gltf.accessors[p.indicesAccessor.value()].count);

            newmesh.firstIndex = cumCount;
            newmesh.indexCount = count;
            newmesh.mesh = fullmesh;
            
            cumCount += count;

            core->engine.defaultMaterial.AddMesh(static_cast<uint32_t>(passManager.meshes.size() - 1));
        }

        meshOffset.push_back(static_cast<uint32_t>(passManager.meshes.size()));
    }

    default_pass::Instance defaultInstace = core->engine.defaultMaterial.GetInstance<default_pass::Pass>();
    ...
    for (fastgltf::Node& node : gltf.nodes)
    {
        ...
        ecs::Entity e = core->engine.entityManager.NewEntity<rendering::Transform>(newNode);

        uint32_t meshIdx = static_cast<uint32_t>(*node.meshIndex);
        uint32_t submeshStart = meshOffset[meshIdx];
        uint32_t submeshEnd = meshOffset[meshIdx + 1];

        for (uint32_t i = submeshStart; i < submeshEnd; ++i)
            passManager.meshes[i].entities.push_back(e);
    }
    core->engine.entityManager.RefreshComponents();
}
