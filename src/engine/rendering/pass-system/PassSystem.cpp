...
namespace rendering
{
    ...
    std::span<Mesh> PassSystem::GetMeshes()
    {
        return meshes;
    }
    ...
    void PassSystem::ReadMeshes(serial::Stream& m)
    {
    ...
    void PassSystem::WriteMeshes(serial::Stream& m)
    {
    ...
    void PassSystem::Serialize(serial::Stream& m)
    {
        if (m.reading)
            ReadMeshes(m);
        else
    ...
    uint32_t PassSystem::AddMesh(Mesh&& mesh, MeshAssetSource&& metaData)
    {
    ...
    uint32_t PassSystem::AddMesh(Mesh&& mesh, MeshDirectSource&& metaData)
    {
    ...
    SubMesh PassSystem::ReferenceMesh(const uint32_t meshIndex, const uint32_t firstIndex /* = 0 */, const uint32_t indexCount /* = ~0u */)
    {
    ...
    void PassSystem::DereferenceMesh(const uint32_t meshIndex)
    {
    ...
    void PassSystem::ReplaceInvalidAssetSources(const assets_system::AssetID sourceAsset)
    {
        assert(meshes.size() == metadata.size());
    ...
    void PassSystem::FixupReferences(const ecs::Engine& passECS)
    {
        refCounts.clear();
