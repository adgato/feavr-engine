...
void ImguiOverlay::Init(const rendering::ResourceHandles& deviceData)
{
    ...
    init_info.PipelineRenderingCreateInfo = vkinit::New<VkPipelineRenderingCreateInfo>(); {
        ...
        init_info.PipelineRenderingCreateInfo.pColorAttachmentFormats = &rendering::imageFormat;
...
void ImguiOverlay::Draw(VkCommandBuffer cmd, rendering::Image& targetImage)
{
    ...
    targetImage.Barrier(cmd, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, barrier);

    //draw imgui into the swapchain image
    const VkRenderingAttachmentInfo targetAttachment = vkinit::attachment_info(targetImage, nullptr);
    const VkRenderingInfo targetRenderInfo = vkinit::rendering_info(
        VkExtent2D { targetImage.imageExtent.width, targetImage.imageExtent.height }, &targetAttachment, nullptr);
